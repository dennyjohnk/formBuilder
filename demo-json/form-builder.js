...
(function ($) {
  var FormBuilder = function FormBuilder(options, element) {
    ...
    // REVIEW: Parse saved JSON template data
    var getJSON = function() {
      var json = new Object();
      if (formBuilder.formData) {
        json = JSON.parse(formBuilder.formData);
      } else if (elem.val() !== '') {
        json = JSON.parse(formBuilder.element.value.trim());
      } else {
        json = false;
      }
...
(function ($) {
  ...
  $.fn.toJSON = function (_helpers) {
    ...
    this.each(function () {
      ...
      if (sortableFields.childNodes.length >= 1) {
        // build new json
        var childLiNodes = $(sortableFields).find('li.form-field');
        // _helpers.forEach(sortableFields.childNodes, function (index, field) {
        _helpers.forEach(childLiNodes, function (index, field) {
          index = index;
          var $field = $(field);
          var fieldData = $field.data('fieldData');
